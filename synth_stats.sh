#!/usr/bin/env bash
# Synthesize one module of the ALU/regfile core for the Basys 3 (Xilinx 7-series) and report resource usage
# Reads the whole rtl/ directory so submodules resolve; the top is the module you name
# sv2v converts the SystemVerilog (package-typed ports, enums) to Verilog-2005 first,
# since Yosys's built-in frontend can't parse package-scoped port types
# Usage: ./synth_stats.sh <top_module>   (e.g. ./synth_stats.sh alu)
set -e
TOP="$1"
